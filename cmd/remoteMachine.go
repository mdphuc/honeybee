...
var remoteMachineCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string){
		...
		if err != nil {
			...
			initLog("remoteMachine.go " + string(stderr_check_docker.Bytes()))
		}else{
		...
		if connect == false && ip != "" && username != ""{
			...
			if container_id == "Not Found"{
				if ip != "" && username != ""{
					...
					initLog("remoteMachine.go " + fmt.Sprintf("%v", err_writefile))
					//Open Docker file
					file, err_openfile := os.OpenFile("./Dockerfile", os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644)
					
					initLog("remoteMachine.go " + fmt.Sprintf("%v", err_openfile))

					err_writefile1 := os.WriteFile("./monitor.service", monitor_service_write, 0644)
					initLog("remoteMachine.go " + fmt.Sprintf("%v", err_writefile1))
					...
					initLog("remoteMachine.go " + string(stderr_dockerbuild.Bytes()))

					blue.Print("==> [In Progress] ")
					white.Print("Starting the environment...\n")
					_ = dockerrun.Run()

					initLog("remoteMachine.go " + string(stderr_dockerrun.Bytes()))
					...
					initLog("remoteMachine.go " + fmt.Sprintf("%v", err_exec))
					initLog("remoteMachine.go " + string(stderr_dockerfile_rm.Bytes()))
					
					write_credential := []byte(fmt.Sprintf("%v@%v", username, ip))
					err_writefile = os.WriteFile("./credential.log", write_credential, 0644)
		...
		if ip == "" && username == "" && compose == false{
			...
			if(container_id != "Not Found"){
				...
				if err == nil{
					...
					var stderr_docker_connect bytes.Buffer
					docker_connect.Stderr = &stderr_docker_connect

					err_conn := docker_connect.Run()
					if err_conn != nil{
						initLog("remoteMachine.go " + string(stderr_docker_connect.Bytes()))
						red.Print("==> [Error] ")
						white.Print(err_conn)	
					}
				}else{
					initLog("remoteMachine.go " + fmt.Sprintf("%v", err))
					red.Print("==> [Error] ")
					white.Print(err)	
				}
			}
